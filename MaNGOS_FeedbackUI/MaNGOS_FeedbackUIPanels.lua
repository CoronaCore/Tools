	...
	Handler = function() if ( event == "PLAYER_LEVEL_UP" ) then 
							for _, line in next, this.infoLines do
								if ( line:GetName():match("Char") ) then 
                                    ...
                                    panel.infoTable["character"] = FeedbackUI_GetLocalizedCharString(arg1, UnitRace("player"), genderTable[UnitSex("player")], UnitClass("player"));
...
--~ 							local moveVariable = -(panel.scroll.maxy - y)
                ...
                panel.StartIncrementalScroll =
                    function(panel, direction)
                        ...
                        this:SetScript("OnUpdate", function() panel.IncrementalUpdate(panel, arg1) end);                
                    end
...
FeedbackUI_SetupPanel{
	...
	Handler = function() if ( event == "PLAYER_LEVEL_UP" ) then 
							for _, line in next, this.infoLines do
								if ( line:GetName():match("Char") ) then 
									line:GetParent().infoTable = line:GetParent().infoTable or {};
									line:GetParent().infoTable["character"] = FeedbackUI_GetLocalizedCharString(arg1, UnitRace("player"), FEEDBACKUI_GENDERTABLE[UnitSex("player")], UnitClass("player"));
									line:GetParent().infoTable["level"] = arg1;
									line:GetParent().infoTable["race"] = UnitRace("player");
									line:GetParent().infoTable["sex"] = FEEDBACKUI_GENDERTABLE[UnitSex("player")];
									line:GetParent().infoTable["class"] = UnitClass("player");
									line.value:SetText(line:GetParent().infoTable["character"])
...
FeedbackUI_SetupPanel{
    ...
    Load =
        function(panel)
            ...
            panel.categories = { "Areas", "Items", "Mobs", "Quests" };
            panel.category = 1;
            panel.maxbuttons = 1;
            panel.status = 2;
            panel.surveys = {};
                    ...
                    for i = 1, GetNumQuestLogEntries() do
                        ...
                        if ( not header ) then                            
                            ...
                            if ( not questFound ) then
                                panel.AddSurvey(panel, { ["name"] = name, ["objectives"] = objectives, ["id"] = objectivesHash, ["type"] = "Quests", ["status"] = "Hidden" })
                            end
                        end
                    end
